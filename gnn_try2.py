...
def build_node_features(G, opinion_key="z", prejudice_key="prejudice"):
    ...
    if X.shape[0] > 0:
        ...
        max_out = max(1.0, float(np.max(X[:, 1])))
...
class GNNEnsembleUCB:
    """
    Neural/GNN Ensemble UCB for top-k user selection.

    No linear reward assumption.

    Each model predicts reward directly:
        r_hat_i = f_theta(A, X)_i

    Selection uses:
        score_i = mean_prediction_i + alpha * ensemble_std_i
    """

    def __init__(self,input_dim,n_models=5,hidden_dim=32,embedding_dim=16,alpha=1.0,lr=1e-3,buffer_size=1000,batch_size=32,train_epochs=1,dropout=0.0,device="cpu",):
        self.device = device
    ...
    def predict_all(self, A_norm, X):
        """
        Returns predictions from all ensemble models.
    ...
    def select_users(self, A_norm, X, k_users,t, epsilon=0.0):
        """
        Select top-k users using ensemble UCB.
        ...
        preds = self.predict_all(A_norm, X)  # shape (M, N)
    ...
    def observe(self, A_norm, X, selected_users, rewards, t):
        """
        Store observed rewards in the sliding buffer.

        Each sample is:
            (A_t, X_t, selected_user, observed_reward)
        """
    ...
    def train(self):
        """
        Train every GNN model on bootstrap samples from the sliding buffer.
...
def gnn_ucb_topk2(G,k,k_f,k_g,M_f,M_g,e_vec,v_vec,T=2000,k_users=5,drift=False,gamma=0.5,K=1,d_target=10,seed=2,alpha=0.5,lr=5e-4,buffer_size=3000,batch_size=64,train_epochs=5,device="cpu",
    ...
    """
    GNN Ensemble UCB with slate-level node features.
    ...
    agent = GNNEnsembleUCB(input_dim=input_dim,n_models=n_models,hidden_dim=hidden_dim,embedding_dim=embedding_dim,alpha=alpha,
